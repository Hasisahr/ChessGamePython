...
def next_turn(snake, food):
    global direction, Rezultat, SPEED
    ...
    if x == food.coordinates[0] and y == food.coordinates[1]:
        Rezultat += 1
        label.config(text="Rezultat:{}".format(Rezultat))
        canvas.delete("food")
        food = Food()
        increase_speed()
    else:
...
def game_over():
    ...
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()/2,
                       font=('consolas', 80), text="GAME OVER", fill="yellow", tag="gameover")
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()/2 + 100,
                       font=('consolas', 40), text="Rezultat: {}".format(Rezultat), fill="white", tag="Rezultat")
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()/2 + 150,
                       font=('consolas', 20), text="Press 'R' to Restart", fill="white", tag="reset")

def restart_game(event):
    global Rezultat, direction, SPEED
    Rezultat = 0
    direction = "down"
    SPEED = 100
    label.config(text="Rezultat: {}".format(Rezultat))
    ...
    canvas.delete("reset")
    canvas.delete("Rezultat")
    snake.__init__()
    food = Food()
    next_turn(snake, food)

window = Tk()
window.title("Zmijica")
window.resizable(False, False)

Rezultat = 0
direction = 'down'

label = Label(window, text="Rezultat:{}".format(Rezultat), font=('consolas', 40))
label.pack()
